...
export function InputRoot({ error = false, ...props }: InputRoot) {
  return (
    <div
      data-error={error}
      className="group flex bg-gray-800 h-12 border border-gray-600 gap-2 px-4 items-center rounded-xl focus-within:border-gray-100 data-[error=true]:border-danger"
      {...props}
    />
  )
}
